const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/catalog-B_KtL5D7.css"])))=>i.map(i=>d[i]);
import{_ as $}from"./preload-helper-CeUGa9z8.js";import{a as A,c as o,P as d}from"./prototypeRoutes-vYiIL7fP.js";const p=[{id:"f01",eyebrow:"CHARACTER FORGE / 2026-08-02",title:"F-01 / Image-to-3D Character Forge",summary:"一枚絵だった主人公を、どの方向から見ても崩れずに動く高密度ボクセルモデルへ。髪、衣装、装備を組み替えられる制作法の最初の形です。",review:"USER REVIEW · APPROX. 70%",path:"forge/f01/"}];function y(t,a){return`${a.endsWith("/")?a:`${a}/`}${t.path}`}const g=[{id:"waf-01",eyebrow:"WORLD ASSET FORGE / 2026-08-14",title:"WAF-01 / 雨水管制塔の生成パイプライン",summary:"建物の意味と配置を保ったまま、BlenderとGeometry Nodesから視覚・衝突・遮蔽用の出力を組み立てる制作事例です。採択済み実画面のみを記録し、runtimeは未統合・未公開のまま保持します。",status:"not-playable",path:"experiments/waf-01/",candidateCommit:"9fb4eed4e55067813c4aa164622a0cb1c7fe1e94",metrics:"830,684 bytes · 3,454 triangles · 7 draws · 5 materials · 512px atlas"}];function _(t,a){return`${a.endsWith("/")?a:`${a}/`}${t.path}`}const h=document.querySelector("#app"),L=["fram-catalog-","relic-frontier-shell-","small-persistent-world-shell-"];if(h===null)throw new Error("Prototype catalog root was not found.");I();const m=A(window.location.search);m!==null?window.location.replace(o(m,"/game/",window.location.search)):(R(h),P(h));function u(){const t=d.find(a=>a.status==="latest")??d[0];if(t===void 0)throw new Error("F.R.A.M. catalog requires a playable build.");return t}function w(t){const[a,i,r]=t.split("-");return`${a}年${Number(i)}月${Number(r)}日`}function R(t){const a=u(),i=o(a.id,"/game/");t.querySelector(".catalog-static__actions a:first-child")?.setAttribute("href",i);const r=t.querySelector(".catalog-static figure img");r!==null&&(r.src=`/game/catalog/${a.id}.jpg`,r.alt=`${a.title} の実画面`);const s=t.querySelector(".catalog-static figcaption");s!==null&&(s.textContent=`LATEST PLAYABLE / ${a.id.toUpperCase()} — 現在のゲーム画面`)}async function P(t){try{await $(()=>Promise.resolve({}),__vite__mapDeps([0])),T(t),C(t)}catch(a){console.error("F.R.A.M. catalog enhancement failed.",a)}}function T(t){const a=u(),i=p[0];if(a===void 0||i===void 0)throw new Error("F.R.A.M. catalog requires a playable build and a technical epoch.");t.className="prototype-catalog",t.dataset.releaseCount=String(d.length),t.dataset.epochCount=String(p.length),t.dataset.pipelineCaseCount=String(g.length);const r=o(a.id,"/game/"),s=`/game/catalog/${a.id}.jpg`,c=d.map(e=>{const n=e.status==="latest",f=n?"LATEST / PLAYABLE":"ARCHIVE / PLAYABLE",v=`/game/catalog/${e.id}.jpg`;return`
      <article class="release-card ${n?"release-card--latest":""}" data-release="${e.id}">
        <div class="release-card__visual">
          <img
            ...
            decoding="async"
          />
          <span class="release-card__status">${f}</span>
        </div>
        <div class="release-card__body">
          <div class="release-card__meta">
            <span class="release-card__id">${e.id.toUpperCase()}</span>
            <time class="release-card__published" datetime="${e.publishedAt}">公開日 ${w(e.publishedAt)}</time>
          </div>
          <h3>${e.title}</h3>
    ...
    `}).join(""),l=p.map(e=>{const n=`/game/catalog/${e.id}.jpg`;return`
      <article class="epoch-card" data-epoch="${e.id}">
        ...
        <div class="epoch-card__body">
          ...
          <h3>${e.title}</h3>
          <p>${e.summary}</p>
          <a class="text-link text-link--gold" href="${y(e,"/game/")}">
            <span>F-01を操作する</span><span aria-hidden="true">↗</span>
          </a>
        </div>
      </article>
    `}).join(""),E=g.map(e=>{const n=`/game/catalog/${e.id}.jpg`;return`
