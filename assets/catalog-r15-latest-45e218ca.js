(() => {
  ...
  const patchStatic = () => {
    ...
    if (image instanceof HTMLImageElement && image.src.endsWith("/catalog/r14.jpg") && !(image.complete && image.naturalWidth > 0)) {
      if (root.dataset.r15CatalogStaticCorrectionWaiting !== "true") {
        root.dataset.r15CatalogStaticCorrectionWaiting = "true";
        image.addEventListener("load", () => {
          delete root.dataset.r15CatalogStaticCorrectionWaiting;
          patchStatic();
        }, { once: true });
      }
      return;
  ...
  const patchLiveCatalog = () => {
    ...
    const initialHeroImage = root.querySelector(".catalog-hero__visual img");
    if (initialHeroImage instanceof HTMLImageElement && initialHeroImage.src.endsWith("/catalog/r14.jpg") && !(initialHeroImage.complete && initialHeroImage.naturalWidth > 0)) {
      if (root.dataset.r15CatalogCorrectionWaiting !== "true") {
        root.dataset.r15CatalogCorrectionWaiting = "true";
        initialHeroImage.addEventListener("load", () => {
          delete root.dataset.r15CatalogCorrectionWaiting;
          patchLiveCatalog();
        }, { once: true });
      }
      return false;
