...
import"./modulepreload-polyfill-B5Qt9EMX.js";import{_ as A}from"./preload-helper-BUwAYG9H.js";import{a as R,c,P as l}from"./prototypeRoutes-DwL0hBdp.js";const g=[{id:"f01",eyebrow:"CHARACTER FORGE / 2026-08-02",title:"F-01 / Image-to-3D Character Forge",summary:"一枚絵だった主人公を、どの方向から見ても崩れずに動く高密度ボクセルモデルへ。髪、衣装、装備を組み替えられる制作法の最初の形です。",review:"USER REVIEW · APPROX. 70%",path:"forge/f01/"}];function b(e,a){return`${a.endsWith("/")?a:`${a}/`}${e.path}`}const h=document.querySelector("#app"),w=["fram-catalog-","relic-frontier-shell-","small-persistent-world-shell-"];if(h===null)throw new Error("Prototype catalog root was not found.");_();const m=R(window.location.search),$=new URLSearchParams(window.location.search).get("prototype");if(m!==null)window.location.replace(c(m,"/game/",window.location.search));else if($==="0.1"){const e="/game/".endsWith("/")?"/game/":"/game//";window.location.replace(`${e}r01/${window.location.search}`)}else L(h),y(h);function f(){const e=l.find(a=>a.status==="latest")??l[0];if(e===void 0)throw new Error("F.R.A.M. catalog requires a playable build.");return e}function L(e){const a=f(),r=c(a.id,"/game/");e.querySelector(".catalog-static__actions a:first-child")?.setAttribute("href",r);const i=e.querySelector(".catalog-static figure img");i!==null&&(i.src=`/game/catalog/${a.id}.jpg`,i.alt=`${a.title} の実画面`);const s=e.querySelector(".catalog-static figcaption");s!==null&&(s.textContent=`LATEST PLAYABLE / ${a.id.toUpperCase()} — 現在のゲーム画面`)}async function y(e){try{await A(()=>Promise.resolve({}),__vite__mapDeps([0])),C(e),I(e)}catch(a){console.error("F.R.A.M. catalog enhancement failed.",a)}}function C(e){const a=f(),r=g[0];if(a===void 0||r===void 0)throw new Error("F.R.A.M. catalog requires a playable build and a technical epoch.");e.className="prototype-catalog",e.dataset.releaseCount=String(l.length),e.dataset.epochCount=String(g.length);const i=c(a.id,"/game/"),s=`/game/catalog/${a.id}.jpg`,n=l.map((t,d)=>{const p=t.status==="latest",E=p?"LATEST / PLAYABLE":"ARCHIVE / PLAYABLE",u=p?"この版を遊ぶ":"保存版を遊ぶ",v=`/game/catalog/${t.id}.jpg`;return`
      <article class="release-card ${p?"release-card--latest":""}" data-release="${t.id}">
        <div class="release-card__visual">
          ...
          <span class="release-card__status">${E}</span>
        </div>
        <div class="release-card__body">
          ...
          <a class="text-link" href="${c(t.id,"/game/")}">
            <span>${u}</span><span aria-hidden="true">↗</span>
          </a>
        </div>
      </article>
    `}).join(""),o=g.map(t=>{const d=`/game/catalog/${t.id}.jpg`;return`
      <article class="epoch-card" data-epoch="${t.id}">
        ...
        <div class="epoch-card__body">
          ...
          <a class="text-link text-link--gold" href="${b(t,"/game/")}">
  ...
  `}function I(e){const a=Array.from(e.querySelectorAll("img[data-deferred-src]")),r=s=>{const n=s.dataset.deferredSrc;n!==void 0&&(s.addEventListener("load",()=>s.classList.add("is-loaded"),{once:!0}),s.src=n,s.removeAttribute("data-deferred-src"))};if(!("IntersectionObserver"in window)){a.forEach(r);return}const i=new IntersectionObserver(s=>{for(const n of s){if(!n.isIntersecting)continue;const o=n.target;i.unobserve(o),r(o)}},{rootMargin:"220px 0px",threshold:.01});a.forEach(s=>i.observe(s))}async function _(){try{if("serviceWorker"in navigator&&await(await navigator.serviceWorker.getRegistration("/game/"))?.unregister(),"caches"in window){const e=await window.caches.keys();await Promise.all(e.filter(a=>w.some(r=>a.startsWith(r))).map(a=>window.caches.delete(a)))}}catch(e){console.warn("F.R.A.M. catalog cache retirement failed.",e)}}
