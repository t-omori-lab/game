...
describe("R04 R02-successor presentation contract", () => {
  ...
  it("maps R04 to its live renderer profile without changing R02 identity", () => {
    ...
    expect(applicationSource).toMatch(
      /environmentProfile:\s*[\s\S]*?options\.experience === "r04"[\s\S]*?options\.experience === "r06"[\s\S]*?\? "r04-live"\s*:\s*options\.experience === "beauty-cell"/,
    );
    expect(applicationSource).toMatch(
      /dataset\.prototypeVersion\s*=\s*r06\s*\? "R06"\s*:\s*r05\s*\? "R05"[\s\S]*?r04\s*\? "R04"/,
    );
