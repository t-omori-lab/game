...
Status: active  
Phase: R09B Playable Character Bridge complete locally, user visual review pending; R09A preserved; R01–R08 preserved
...
- 2026-08-08、R09B `Playable Character Bridge`をlocal実装した。F-01のcompiled 9,454-cell surface packを薄いR09専用dynamic actor adapterで通常camera、world light、移動、4方向、装備、manual skill、auto-basicへ接続し、実画面でfailしたface、torso/jacket、limbs、backpack、combat poseだけへ706 cellsを追加したF-02を作った。ForgeとR09は同じ`fram.character.f02.gameplay-readability-v1`／10,160 cellsを使い、1280×720／2560×1440のW/A/S/D実移動・simulation facing・rendered yaw、Forge back／FIELD、旧R05 actor fallbackでconsole／page error 0だった。R09Aの四分岐、二周目差分、reload、retreatも再合格した。詳細voxelのgameplay shadow passを既存blob shadowへ集約し、local desktop Chrome 3-run medianはR06／R09でfirst-controllable 979.2／940.5 ms、transfer 789,846／832,495 B、frame p95 33.8／35.0 ms、long frame 0となり10%回帰gateを通過した。strict TypeScript、Vitest 43 files／231 tests、production buildも合格した。これはlocal visual review candidateであり、ユーザー造形採択、commercial quality、iPhone 16 Pro、public deployではない。
- 2026-08-08、R09A `First Memory Logic Proof`をlocal完了した。`WorldEvent → pure reducer → WorldMemoryState v1 → strict codec → R09専用A/B local save`を現在遠征の`PrototypeBState`から分離し、二site、回収物消費、拠点確保、二module、撤退、二回目の因果差分を同じR06由来sceneへ接続した。Pathfinder Arrayはroute表示と探索速度1.12倍、Relic Overdriveはcoral auraと大技cooldown 0.65倍を二回目開始直後に示す。desktop Chromeのproduction previewで四分岐すべてを実移動／auto-basic／手動大技込みで完走し、reload後の復旧、撤退時の発見site／回収物だけの保持、console／page error 0を確認した。strict TypeScript、Vitest 39 files／217 tests、production buildに合格した。R09対R06のlocal測定はfirst-controllable 988.0／985.0 ms、same-origin transfer 794,338／789,565 bytes、frame p95 18.5／18.6 ms、50ms超long frame 0で、10%回帰gateを通過した。これはlocal logic／browser proofであり、public deploy、iPhone 16 Pro、F-01／F-02、user gameplay／visual acceptanceではない。
...
- 本release candidateはstrict TypeScript、Vitest 38 files／205 tests、production build、desktop 1280×720／mobile 390×844 browser QAに合格した。commit `026970f`を`main`へpushし、GitHub Pages run #21のbuild／deploy成功を確認した。公開catalogはgame-first first view、hero 1件だけの初期`src`、archive 7件のdeferred状態を維持し、公開ForgeのFIELDはactor画面高16.2%だった。catalog／Forgeともconsole error／warning 0件で、`/game/`、`/game/r01/`、`/game/r06/`、`/game/forge/f01/`はHTTPS 200だった。iPhone 16 Pro実機とuser visual acceptanceは未確認である。
