...
- 高密度voxel actorをproduction sceneへ入れると、visible cell数よりも、rig part × materialごとのInstancedMeshをshadow mapへ再描画する費用がframe pacingを支配し得る。R09Bでは10,160 cellsを維持したまま、gameplay actorの詳細shadow casterを既存blob shadowへ集約すると、frame p95が50.0 msから34.2 ms、50ms超frameが17から0へ戻った。Forge close-upではauthored shadowを維持し、制作確認とplay budgetを分ける。
