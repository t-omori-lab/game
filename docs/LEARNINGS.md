...
- release一覧へ新しいRXXを追加すると、routeと説明が正しくても`public/catalog/<id>.jpg`がなければdeferred imageだけが404になり、カード背景が空のまま残る。release manifestとthumbnailを同じ変更単位にし、全宣言IDの画像存在と最低sizeをtestすると、最新版カードだけの欠落をbuild時に止められる。
...
- 既に承認済みcompiled 3D packが存在するassetをgameへ移すとき、画像から再推定した別packを「忠実版」として既定化してはいけない。正本factoryとpayloadを無加工で共有し、game adapterはscale、facing、socket、motion、shadowだけを担当する。改善版は正本を凍結した別versionとして比較する。
...
- 高密度voxel actorをproduction sceneへ入れると、visible cell数よりも、rig part × materialごとのInstancedMeshをshadow mapへ再描画する費用がframe pacingを支配し得る。R09Bでは10,160 cellsを維持したまま、gameplay actorの詳細shadow casterを既存blob shadowへ集約すると、frame p95が50.0 msから最終35.0 ms、50ms超frameが17から0へ戻った。Forge close-upではauthored shadowを維持し、制作確認とplay budgetを分ける。
- 画像生成から作ったcharacterをStandalone Forgeだけで評価せず、同じcompiled packを通常camera、world light、装備、移動、auto-basic、manual skillへ薄いadapterで接続すると、造形とruntime契約の失敗を分離できる。F-01ではsocket scale、face、coat、limbs、pack、combat poseだけが実画面で露呈し、合格済み9,454 cellsを置換せず706-cellのF-02へ限定できた。close-upの印象や総cell数を、gameplay採択の代用にしない。
