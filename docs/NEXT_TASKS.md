...
catalog先頭の「01」カードに欠けていたR09 thumbnailは公開済みで、全releaseの画像存在検査も追加した。公開画像のSHA-256一致、実browserでの可視表示、入口と過去版routeの200応答まで確認済みである。
...
- [x] scope限定commitをpushし、GitHub Pagesの`/game/`で01カードと画像応答を確認する。
