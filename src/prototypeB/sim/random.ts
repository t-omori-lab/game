import type { PrototypeBSeed } from "./types";
...
export function normalizePrototypeBSeed(seed: PrototypeBSeed): number {
  const normalized =
...
export interface PrototypeBRandomDraw {
  state: number;
  value: number;
}

export function nextPrototypeBRandom(
  randomState: number,
): PrototypeBRandomDraw {
